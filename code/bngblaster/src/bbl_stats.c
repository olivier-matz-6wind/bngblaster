...
bbl_stats_json(bbl_stats_s * stats)
{
    ...
    if(sizeof(BNGBLASTER_VERSION)-1) {
        json_object_set_new(jobj, "version", json_string(BNGBLASTER_VERSION));
    } else {
        json_object_set_new(jobj, "version", json_string("DEV"));
    }

    json_object_set_new(jobj, "test-duration", json_integer(test_duration()));
    if(g_ctx->sessions) {
        ...
        json_object_set_new(jobj, "setup-time-ms", json_integer(g_ctx->stats.setup_time));
        json_object_set_new(jobj, "setup-rate-cps", json_real(g_ctx->stats.cps));
        json_object_set_new(jobj, "setup-rate-cps-min", json_real(g_ctx->stats.cps_min));
        json_object_set_new(jobj, "setup-rate-cps-avg", json_real(g_ctx->stats.cps_avg));
        json_object_set_new(jobj, "setup-rate-cps-max", json_real(g_ctx->stats.cps_max));
        json_object_set_new(jobj, "dhcp-sessions-established", json_integer(g_ctx->dhcp_established_max));
        json_object_set_new(jobj, "dhcpv6-sessions-established", json_integer(g_ctx->dhcpv6_established_max));
    }
    if(dict_count(g_ctx->li_flow_dict)) {
        jobj_sub = json_object();
        json_object_set_new(jobj_sub, "flows", json_integer(dict_count(g_ctx->li_flow_dict)));
    ...
    if(g_ctx->config.l2tp_server || g_ctx->config.l2tp_client) {
        ...
        json_object_set_new(jobj_sub, "sessions", json_integer(g_ctx->l2tp_sessions_max));
        json_object_set_new(jobj_sub, "tx-control-packets", json_integer(stats->l2tp_control_tx));
        json_object_set_new(jobj_sub, "tx-control-packets-retry", json_integer(stats->l2tp_control_retry));
        json_object_set_new(jobj_sub, "rx-control-packets", json_integer(stats->l2tp_control_rx));
        json_object_set_new(jobj_sub, "rx-control-packets-duplicate", json_integer(stats->l2tp_control_rx_dup));
        json_object_set_new(jobj_sub, "rx-control-packets-out-of-order", json_integer(stats->l2tp_control_rx_ooo));
        json_object_set_new(jobj_sub, "tx-data-packets", json_integer(stats->l2tp_data_tx));
        json_object_set_new(jobj_sub, "rx-data-packets", json_integer(stats->l2tp_data_rx));
        json_object_set_new(jobj, "l2tp", jobj_sub);
    }
    ...
    CIRCLEQ_FOREACH(interface, &g_ctx->interface_qhead, interface_qnode) {
        jobj_sub = json_object();
        json_object_set_new(jobj_sub, "name", json_string(interface->name));
        json_object_set_new(jobj_sub, "type", json_string(interface_type_string(interface->type)));
        if(interface->type != LAG_INTERFACE) {
            bbl_stats_generate_interface(interface->io.tx, &interface_stats_tx);
            bbl_stats_generate_interface(interface->io.rx, &interface_stats_rx);

            json_object_set_new(jobj_sub, "tx-packets", json_integer(interface_stats_tx.packets));
            json_object_set_new(jobj_sub, "tx-bytes", json_integer(interface_stats_tx.bytes));
            json_object_set_new(jobj_sub, "tx-polled", json_integer(interface_stats_tx.polled));
        ...
        if(interface->type == LAG_MEMBER_INTERFACE && 
           interface->lag_member->lacp_state) {
            ...
            json_object_set_new(jobj_sub2, "tx", json_integer(interface->lag_member->stats.lacp_tx));
            json_object_set_new(jobj_sub2, "rx", json_integer(interface->lag_member->stats.lacp_rx));
            json_object_set_new(jobj_sub2, "dropped", json_integer(interface->lag_member->stats.lacp_dropped));
            json_object_set_new(jobj_sub, "lacp", jobj_sub2);
        }
        json_array_append_new(jobj_array, jobj_sub);
        array_size++;
    }
    json_object_set_new(jobj, "interfaces", jobj_array);
    ...
    CIRCLEQ_FOREACH(interface, &g_ctx->interface_qhead, interface_qnode) {
        ...
        while(network_interface) {
            jobj_sub = json_object();
            json_object_set_new(jobj_sub, "name", json_string(network_interface->name));
            json_object_set_new(jobj_sub, "tx-packets", json_integer(network_interface->stats.packets_tx));
            json_object_set_new(jobj_sub, "tx-multicast-packets", json_integer(network_interface->stats.mc_tx));
            json_object_set_new(jobj_sub, "rx-packets", json_integer(network_interface->stats.packets_rx));
            if(g_ctx->stats.session_traffic_flows) {
                json_object_set_new(jobj_sub, "tx-stream-packets", json_integer(network_interface->stats.stream_tx));
                json_object_set_new(jobj_sub, "rx-stream-packets", json_integer(network_interface->stats.stream_rx));
                json_object_set_new(jobj_sub, "rx-stream-packets-loss", json_integer(network_interface->stats.stream_loss));
            }
            if(g_ctx->stats.session_traffic_flows) {
                json_object_set_new(jobj_sub, "tx-session-packets-ipv4", json_integer(network_interface->stats.session_ipv4_tx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv4", json_integer(network_interface->stats.session_ipv4_rx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv4-loss", json_integer(network_interface->stats.session_ipv4_loss));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv4-avg-pps-max", json_integer(network_interface->stats.rate_session_ipv4_tx.avg_max));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv4-avg-pps-max", json_integer(network_interface->stats.rate_session_ipv4_rx.avg_max));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv6", json_integer(network_interface->stats.session_ipv6_tx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6", json_integer(network_interface->stats.session_ipv6_rx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6-loss", json_integer(network_interface->stats.session_ipv6_loss));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv6-avg-pps-max", json_integer(network_interface->stats.rate_session_ipv6_tx.avg_max));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6-avg-pps-max", json_integer(network_interface->stats.rate_session_ipv6_rx.avg_max));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv6pd", json_integer(network_interface->stats.session_ipv6pd_tx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6pd", json_integer(network_interface->stats.session_ipv6pd_rx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6pd-loss", json_integer(network_interface->stats.session_ipv6pd_loss));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv6pd-avg-pps-max", json_integer(network_interface->stats.rate_session_ipv6pd_tx.avg_max));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6pd-avg-pps-max", json_integer(network_interface->stats.rate_session_ipv6pd_rx.avg_max));
            }
            json_array_append_new(jobj_array, jobj_sub);
            array_size++;
            network_interface = network_interface->next;
    ...
    if(array_size) {
        json_object_set_new(jobj, "network-interfaces", jobj_array);
    } else {
    ...
    CIRCLEQ_FOREACH(interface, &g_ctx->interface_qhead, interface_qnode) {
        ...
        if(access_interface) {
            jobj_sub = json_object();
            json_object_set_new(jobj_sub, "name", json_string(interface->name));
            json_object_set_new(jobj_sub, "tx-packets", json_integer(access_interface->stats.packets_tx));
            json_object_set_new(jobj_sub, "rx-packets", json_integer(access_interface->stats.packets_rx));
            json_object_set_new(jobj_sub, "rx-multicast-packets", json_integer(access_interface->stats.mc_rx));
            json_object_set_new(jobj_sub, "rx-multicast-packets-loss", json_integer(access_interface->stats.mc_loss));
            if(g_ctx->stats.session_traffic_flows) {
                json_object_set_new(jobj_sub, "tx-stream-packets", json_integer(access_interface->stats.stream_tx));
                json_object_set_new(jobj_sub, "rx-stream-packets", json_integer(access_interface->stats.stream_rx));
                json_object_set_new(jobj_sub, "rx-stream-packets-loss", json_integer(access_interface->stats.stream_loss));
            }
            if(g_ctx->stats.session_traffic_flows) {
                json_object_set_new(jobj_sub, "tx-session-packets-ipv4", json_integer(access_interface->stats.session_ipv4_tx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv4", json_integer(access_interface->stats.session_ipv4_rx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv4-loss", json_integer(access_interface->stats.session_ipv4_loss));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv4-wrong-session", json_integer(access_interface->stats.session_ipv4_wrong_session));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv4-avg-pps-max", json_integer(access_interface->stats.rate_session_ipv4_tx.avg_max));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv4-avg-pps-max", json_integer(access_interface->stats.rate_session_ipv4_rx.avg_max));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv6", json_integer(access_interface->stats.session_ipv6_tx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6", json_integer(access_interface->stats.session_ipv6_rx));
                ...
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6pd-wrong-session", json_integer(access_interface->stats.session_ipv6pd_wrong_session));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv6pd-avg-pps-max", json_integer(access_interface->stats.rate_session_ipv6pd_tx.avg_max));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6pd-avg-pps-max", json_integer(access_interface->stats.rate_session_ipv6pd_rx.avg_max));
            }
            jobj_sub2 = json_object();
            json_object_set_new(jobj_sub2, "tx-arp", json_integer(access_interface->stats.arp_tx));
            json_object_set_new(jobj_sub2, "rx-arp", json_integer(access_interface->stats.arp_rx));
            json_object_set_new(jobj_sub2, "tx-padi", json_integer(access_interface->stats.padi_tx));
            json_object_set_new(jobj_sub2, "rx-pado", json_integer(access_interface->stats.pado_rx));
            json_object_set_new(jobj_sub2, "tx-padr", json_integer(access_interface->stats.padr_tx));
            json_object_set_new(jobj_sub2, "rx-pads", json_integer(access_interface->stats.pads_rx));
            json_object_set_new(jobj_sub2, "tx-padt", json_integer(access_interface->stats.padt_tx));
            json_object_set_new(jobj_sub2, "rx-padt", json_integer(access_interface->stats.padt_rx));
            json_object_set_new(jobj_sub2, "tx-lcp", json_integer(access_interface->stats.lcp_tx));
            json_object_set_new(jobj_sub2, "rx-lcp", json_integer(access_interface->stats.lcp_rx));
            json_object_set_new(jobj_sub2, "tx-pap", json_integer(access_interface->stats.pap_tx));
            json_object_set_new(jobj_sub2, "rx-pap", json_integer(access_interface->stats.pap_rx));
            json_object_set_new(jobj_sub2, "tx-chap", json_integer(access_interface->stats.chap_tx));
            json_object_set_new(jobj_sub2, "rx-chap", json_integer(access_interface->stats.chap_rx));
            json_object_set_new(jobj_sub2, "tx-ipcp", json_integer(access_interface->stats.ipcp_tx));
            json_object_set_new(jobj_sub2, "rx-ipcp", json_integer(access_interface->stats.ipcp_rx));
            json_object_set_new(jobj_sub2, "tx-ip6cp", json_integer(access_interface->stats.ip6cp_tx));
            json_object_set_new(jobj_sub2, "rx-ip6cp", json_integer(access_interface->stats.ip6cp_rx));
            json_object_set_new(jobj_sub2, "tx-igmp", json_integer(access_interface->stats.igmp_tx));
            json_object_set_new(jobj_sub2, "rx-igmp", json_integer(access_interface->stats.igmp_rx));
            json_object_set_new(jobj_sub2, "tx-icmp", json_integer(access_interface->stats.icmp_tx));
            json_object_set_new(jobj_sub2, "rx-icmp", json_integer(access_interface->stats.icmp_rx));
            json_object_set_new(jobj_sub2, "tx-dhcp", json_integer(access_interface->stats.dhcp_tx));
            json_object_set_new(jobj_sub2, "rx-dhcp", json_integer(access_interface->stats.dhcp_rx));
            json_object_set_new(jobj_sub2, "tx-dhcpv6", json_integer(access_interface->stats.dhcpv6_tx));
            json_object_set_new(jobj_sub2, "rx-dhcpv6", json_integer(access_interface->stats.dhcpv6_rx));
            json_object_set_new(jobj_sub2, "tx-icmpv6", json_integer(access_interface->stats.icmpv6_tx));
            json_object_set_new(jobj_sub2, "rx-icmpv6", json_integer(access_interface->stats.icmpv6_rx));
            json_object_set_new(jobj_sub2, "rx-ipv4-fragmented", json_integer(access_interface->stats.ipv4_fragmented_rx));
            json_object_set_new(jobj_sub2, "lcp-echo-timeout", json_integer(access_interface->stats.lcp_echo_timeout));
            json_object_set_new(jobj_sub2, "lcp-request-timeout", json_integer(access_interface->stats.lcp_timeout));
            json_object_set_new(jobj_sub2, "ipcp-request-timeout", json_integer(access_interface->stats.ipcp_timeout));
            json_object_set_new(jobj_sub2, "ip6cp-request-timeout", json_integer(access_interface->stats.ip6cp_timeout));
            json_object_set_new(jobj_sub2, "pap-timeout", json_integer(access_interface->stats.pap_timeout));
            json_object_set_new(jobj_sub2, "chap-timeout", json_integer(access_interface->stats.chap_timeout));
            json_object_set_new(jobj_sub2, "dhcp-timeout", json_integer(access_interface->stats.dhcp_timeout));
            json_object_set_new(jobj_sub2, "dhcpv6-timeout", json_integer(access_interface->stats.dhcpv6_timeout));
            json_object_set_new(jobj_sub2, "icmpv6-rs-timeout", json_integer(access_interface->stats.dhcpv6_timeout));
            json_object_set_new(jobj_sub, "protocol-stats", jobj_sub2);
            json_array_append_new(jobj_array, jobj_sub);
            array_size++;
        }
    }
    if(array_size) {
        json_object_set_new(jobj, "access-interfaces", jobj_array);
    } else {
        json_decref(jobj_array);
    }
    ...
    CIRCLEQ_FOREACH(interface, &g_ctx->interface_qhead, interface_qnode) {
        ...
        if(a10nsp_interface) {
            jobj_sub = json_object();
            json_object_set_new(jobj_sub, "name", json_string(interface->name));
            json_object_set_new(jobj_sub, "tx-packets", json_integer(a10nsp_interface->stats.packets_tx));
            json_object_set_new(jobj_sub, "rx-packets", json_integer(a10nsp_interface->stats.packets_rx));
            if(g_ctx->stats.session_traffic_flows) {
                json_object_set_new(jobj_sub, "tx-stream-packets", json_integer(a10nsp_interface->stats.stream_tx));
                json_object_set_new(jobj_sub, "rx-stream-packets", json_integer(a10nsp_interface->stats.stream_rx));
                json_object_set_new(jobj_sub, "rx-stream-packets-loss", json_integer(a10nsp_interface->stats.stream_loss));
            }
            if(g_ctx->stats.session_traffic_flows) {
                json_object_set_new(jobj_sub, "tx-session-packets-ipv4", json_integer(a10nsp_interface->stats.session_ipv4_tx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv4", json_integer(a10nsp_interface->stats.session_ipv4_rx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv4-loss", json_integer(a10nsp_interface->stats.session_ipv4_loss));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv4-avg-pps-max", json_integer(a10nsp_interface->stats.rate_session_ipv4_tx.avg_max));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv4-avg-pps-max", json_integer(a10nsp_interface->stats.rate_session_ipv4_rx.avg_max));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv6", json_integer(a10nsp_interface->stats.session_ipv6_tx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6", json_integer(a10nsp_interface->stats.session_ipv6_rx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6-loss", json_integer(a10nsp_interface->stats.session_ipv6_loss));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv6-avg-pps-max", json_integer(a10nsp_interface->stats.rate_session_ipv6_tx.avg_max));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6-avg-pps-max", json_integer(a10nsp_interface->stats.rate_session_ipv6_rx.avg_max));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv6pd", json_integer(a10nsp_interface->stats.session_ipv6pd_tx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6pd", json_integer(a10nsp_interface->stats.session_ipv6pd_rx));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6pd-loss", json_integer(a10nsp_interface->stats.session_ipv6pd_loss));
                json_object_set_new(jobj_sub, "tx-session-packets-ipv6pd-avg-pps-max", json_integer(a10nsp_interface->stats.rate_session_ipv6pd_tx.avg_max));
                json_object_set_new(jobj_sub, "rx-session-packets-ipv6pd-avg-pps-max", json_integer(a10nsp_interface->stats.rate_session_ipv6pd_rx.avg_max));
            }
            json_array_append_new(jobj_array, jobj_sub);
            array_size++;
        }
    ...
    if(array_size) {
        json_object_set_new(jobj, "a10nsp-interfaces", jobj_array);
    } else {
        json_decref(jobj_array);
    }
    ...
    if(g_ctx->stats.session_traffic_flows) {
        jobj_sub = json_object();
        json_object_set_new(jobj_sub, "config-ipv4-pps", json_integer(g_ctx->config.session_traffic_ipv4_pps));
        json_object_set_new(jobj_sub, "config-ipv6-pps", json_integer(g_ctx->config.session_traffic_ipv6_pps));
        json_object_set_new(jobj_sub, "config-ipv6pd-pps", json_integer(g_ctx->config.session_traffic_ipv6pd_pps));
        json_object_set_new(jobj_sub, "total-flows", json_integer(g_ctx->stats.session_traffic_flows));

        json_object_set_new(jobj_sub, "verified-flows", json_integer(g_ctx->stats.session_traffic_flows_verified));
        json_object_set_new(jobj_sub, "verified-flows-downstream-ipv4", json_integer(stats->sessions_down_ipv4_rx));
        json_object_set_new(jobj_sub, "verified-flows-downstream-ipv6", json_integer(stats->sessions_down_ipv6_rx));
        json_object_set_new(jobj_sub, "verified-flows-downstream-ipv6pd", json_integer(stats->sessions_down_ipv6pd_rx));
        json_object_set_new(jobj_sub, "verified-flows-upstream-ipv4", json_integer(stats->sessions_up_ipv4_rx));
        json_object_set_new(jobj_sub, "verified-flows-upstream-ipv6", json_integer(stats->sessions_up_ipv6_rx));
        json_object_set_new(jobj_sub, "verified-flows-upstream-ipv6pd", json_integer(stats->sessions_up_ipv6pd_rx));

        json_object_set_new(jobj_sub, "violated-flows-downstream-ipv4-3s", json_integer(stats->violations_down_ipv4_3s));
        json_object_set_new(jobj_sub, "violated-flows-downstream-ipv6-3s", json_integer(stats->violations_down_ipv6_3s));
        json_object_set_new(jobj_sub, "violated-flows-downstream-ipv6pd-3s", json_integer(stats->violations_down_ipv6pd_3s));
        json_object_set_new(jobj_sub, "violated-flows-upstream-ipv4-3s", json_integer(stats->violations_up_ipv4_3s));
        json_object_set_new(jobj_sub, "violated-flows-upstream-ipv6-3s", json_integer(stats->violations_up_ipv6_3s));
        json_object_set_new(jobj_sub, "violated-flows-upstream-ipv6pd-3s", json_integer(stats->violations_up_ipv6pd_3s));

        json_object_set_new(jobj_sub, "violated-flows-downstream-ipv4-2s", json_integer(stats->violations_down_ipv4_2s));
        json_object_set_new(jobj_sub, "violated-flows-downstream-ipv6-2s", json_integer(stats->violations_down_ipv6_2s));
        json_object_set_new(jobj_sub, "violated-flows-downstream-ipv6pd-2s", json_integer(stats->violations_down_ipv6pd_2s));
        json_object_set_new(jobj_sub, "violated-flows-upstream-ipv4-2s", json_integer(stats->violations_up_ipv4_2s));
        json_object_set_new(jobj_sub, "violated-flows-upstream-ipv6-2s", json_integer(stats->violations_up_ipv6_2s));
        json_object_set_new(jobj_sub, "violated-flows-upstream-ipv6pd-2s", json_integer(stats->violations_up_ipv6pd_2s));

        json_object_set_new(jobj_sub, "violated-flows-downstream-ipv4-1s", json_integer(stats->violations_down_ipv4_1s));
        json_object_set_new(jobj_sub, "violated-flows-downstream-ipv6-1s", json_integer(stats->violations_down_ipv6_1s));
        json_object_set_new(jobj_sub, "violated-flows-downstream-ipv6pd-1s", json_integer(stats->violations_down_ipv6pd_1s));
        json_object_set_new(jobj_sub, "violated-flows-upstream-ipv4-1s", json_integer(stats->violations_up_ipv4_1s));
        json_object_set_new(jobj_sub, "violated-flows-upstream-ipv6-1s", json_integer(stats->violations_up_ipv6_1s));
        json_object_set_new(jobj_sub, "violated-flows-upstream-ipv6pd-1s", json_integer(stats->violations_up_ipv6pd_1s));

        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv4-min", json_integer(stats->min_down_ipv4_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv4-avg", json_integer(stats->avg_down_ipv4_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv4-max", json_integer(stats->max_down_ipv4_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6-min", json_integer(stats->min_down_ipv6_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6-avg", json_integer(stats->avg_down_ipv6_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6-max", json_integer(stats->max_down_ipv6_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6pd-min", json_integer(stats->min_down_ipv6pd_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6pd-avg", json_integer(stats->avg_down_ipv6pd_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6pd-max", json_integer(stats->max_down_ipv6pd_rx_first_seq));

        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv4-min", json_integer(stats->min_up_ipv4_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv4-avg", json_integer(stats->avg_up_ipv4_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv4-max", json_integer(stats->max_up_ipv4_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6-min", json_integer(stats->min_up_ipv6_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6-avg", json_integer(stats->avg_up_ipv6_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6-max", json_integer(stats->max_up_ipv6_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6pd-min", json_integer(stats->min_up_ipv6pd_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6pd-avg", json_integer(stats->avg_up_ipv6pd_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6pd-max", json_integer(stats->max_up_ipv6pd_rx_first_seq));

        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv4-min-seconds", json_real(stats->min_down_ipv4_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv4-avg-seconds", json_real(stats->avg_down_ipv4_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv4-max-seconds", json_real(stats->max_down_ipv4_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6-min-seconds", json_real(stats->min_down_ipv6_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6-avg-seconds", json_real(stats->avg_down_ipv6_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6-max-seconds", json_real(stats->max_down_ipv6_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6pd-min-seconds", json_real(stats->min_down_ipv6pd_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6pd-avg-seconds", json_real(stats->avg_down_ipv6pd_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-downstream-ipv6pd-max-seconds", json_real(stats->max_down_ipv6pd_rx_seconds));

        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv4-min-seconds", json_real(stats->min_up_ipv4_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv4-avg-seconds", json_real(stats->avg_up_ipv4_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv4-max-seconds", json_real(stats->max_up_ipv4_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6-min-seconds", json_real(stats->min_up_ipv6_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6-avg-seconds", json_real(stats->avg_up_ipv6_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6-max-seconds", json_real(stats->max_up_ipv6_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6pd-min-seconds", json_real(stats->min_up_ipv6pd_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6pd-avg-seconds", json_real(stats->avg_up_ipv6pd_rx_seconds));
        json_object_set_new(jobj_sub, "first-seq-rx-upstream-ipv6pd-max-seconds", json_real(stats->max_up_ipv6pd_rx_seconds));

        json_object_set_new(jobj, "session-traffic", jobj_sub);
    }

    if(g_ctx->stats.stream_traffic_flows) {
        jobj_sub = json_object();
        json_object_set_new(jobj_sub, "total-flows", json_integer(g_ctx->stats.stream_traffic_flows));
        json_object_set_new(jobj_sub, "verified-flows", json_integer(g_ctx->stats.stream_traffic_flows_verified));
        json_object_set_new(jobj_sub, "first-seq-rx-min", json_integer(stats->min_stream_rx_first_seq));
        json_object_set_new(jobj_sub, "first-seq-rx-max", json_integer(stats->max_stream_rx_first_seq));
        json_object_set_new(jobj_sub, "flow-rx-packet-loss-min", json_integer(stats->min_stream_loss));
        json_object_set_new(jobj_sub, "flow-rx-packet-loss-max", json_integer(stats->max_stream_loss));
        json_object_set_new(jobj_sub, "flow-rx-delay-us-min", json_integer(stats->min_stream_delay_us));
        json_object_set_new(jobj_sub, "flow-rx-delay-us-max", json_integer(stats->max_stream_delay_us));
        json_object_set_new(jobj, "traffic-streams", jobj_sub);
    }

    if(g_ctx->config.igmp_group_count > 1) {
        jobj_sub = json_object();
        json_object_set_new(jobj_sub, "config-version", json_integer(g_ctx->config.igmp_version));
        json_object_set_new(jobj_sub, "config-start-delay", json_integer(g_ctx->config.igmp_start_delay));
        json_object_set_new(jobj_sub, "config-group-count", json_integer(g_ctx->config.igmp_group_count));
        json_object_set_new(jobj_sub, "config-zapping-interval", json_integer(g_ctx->config.igmp_zap_interval));
        json_object_set_new(jobj_sub, "config-zapping-count", json_integer(g_ctx->config.igmp_zap_count));
        json_object_set_new(jobj_sub, "config-zapping-view-duration", json_integer(g_ctx->config.igmp_zap_view_duration));
        if(g_ctx->config.igmp_zap_interval > 0) {
            json_object_set_new(jobj_sub, "zapping-join-delay-ms-min", json_integer(stats->min_join_delay));
            json_object_set_new(jobj_sub, "zapping-join-delay-ms-avg", json_integer(stats->avg_join_delay));
            json_object_set_new(jobj_sub, "zapping-join-delay-ms-max", json_integer(stats->max_join_delay));
            if(g_ctx->config.igmp_max_join_delay) {
                json_object_set_new(jobj_sub, "zapping-join-delay-violations", json_integer(stats->join_delay_violations));
                json_object_set_new(jobj_sub, "zapping-join-delay-violations-threshold", json_integer(g_ctx->config.igmp_max_join_delay));
            }
            json_object_set_new(jobj_sub, "zapping-join-delay-violations-125ms", json_integer(stats->join_delay_violations_125ms));
            json_object_set_new(jobj_sub, "zapping-join-delay-violations-250ms", json_integer(stats->join_delay_violations_250ms));
            json_object_set_new(jobj_sub, "zapping-join-delay-violations-500ms", json_integer(stats->join_delay_violations_500ms));
            json_object_set_new(jobj_sub, "zapping-join-delay-violations-1s", json_integer(stats->join_delay_violations_1s));
            json_object_set_new(jobj_sub, "zapping-join-delay-violations-2s", json_integer(stats->join_delay_violations_2s));
            json_object_set_new(jobj_sub, "zapping-join-count", json_integer(stats->zapping_join_count));
            json_object_set_new(jobj_sub, "zapping-leave-delay-ms-min", json_integer(stats->min_leave_delay));
            json_object_set_new(jobj_sub, "zapping-leave-delay-ms-avg", json_integer(stats->avg_leave_delay));
            json_object_set_new(jobj_sub, "zapping-leave-delay-ms-max", json_integer(stats->max_leave_delay));
            json_object_set_new(jobj_sub, "zapping-leave-count", json_integer(stats->zapping_leave_count));
            json_object_set_new(jobj_sub, "zapping-multicast-packets-overlap", json_integer(stats->mc_old_rx_after_first_new));
            json_object_set_new(jobj_sub, "zapping-multicast-not-received", json_integer(stats->mc_not_received));
        }
        json_object_set_new(jobj, "multicast", jobj_sub);
    }
    ...
    if(g_ctx->config.json_report_sessions) {
        ...
        json_object_set_new(jobj, "sessions", jobj_array);
    }
    ...
    if(g_ctx->config.json_report_streams) {
        ...
        json_object_set_new(jobj, "streams", jobj_array);
    }

    json_object_set_new(root, "report", jobj);
