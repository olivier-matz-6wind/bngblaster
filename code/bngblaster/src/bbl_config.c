...
json_parse_config(json_t *root)
{
    ...
    if(json_is_object(section)) {
        ...
        JSON_OBJ_GET_BOOL(section, value, "dhcp", "keep-address");
        if(value) {
            g_ctx->config.dhcp_keep_address = json_boolean_value(value);
        }
...
bbl_config_init_defaults()
{
    ...
    g_ctx->config.dhcp_keep_address = false;
